...
test.describe("OG-метаданные на главной", () => {
  test("на главной есть og:title, og:description, og:url, og:image", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(
    ...
    await expect(
      page.locator('meta[property="og:url"]').first(),
    ).toHaveAttribute("content", /.+/);
    await expect(
  ...
  test("на главной есть twitter:card, twitter:title, twitter:description, twitter:image", async ({
    ...
    await page.goto("/");
    await expect(
      page.locator('meta[name="twitter:card"]').first(),
    ).toHaveAttribute("content", /.+/);
    await expect(
      page.locator('meta[name="twitter:title"]').first(),
    ).toHaveAttribute("content", /.+/);
    await expect(
  ...
  test("og:title содержит название сайта или заголовок страницы", async ({
    page,
  }) => {
    await page.goto("/");

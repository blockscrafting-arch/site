...
- **SEO already integrated**: `@astrojs/sitemap`, `robots.txt` (static `public/`), JSON-LD schema (in `BaseLayout`), Yandex Metrika (ID in `site.yandexMetrikaId`, noscript fallback in `BaseLayout`).

...
- **Homepage Hero height**: `Hero.astro` uses `min-h-[calc(100svh-4rem)]` on the outer `<section>` so the first screen fills the viewport minus the fixed header (`4rem` = `h-16`). Uses `100svh` (small viewport height) for predictable mobile behavior (e.g. iOS). Without this, the next section (e.g. ProblemSolution) was visible above the fold and looked like a layout bug.
...
- **Публичный ориентир по бюджету**: «от 5 000 р» — Hero, about, llms.txt, контент кейсов по необходимости. **Страницы `/services` и `/services/[id]` не показывают цены в UI** (карточки, hero, ссылки на кейсы). В `src/data/services.ts` поля `price` остаются для JSON-LD `Offer` на детальных услугах. Подробно: `docs/SERVICES.md`.
...
- **TLS renewal on VPS**: `certbot` renews `vladexecute.ru` with `webroot_path=/var/www/certbot`. Nginx must serve `/.well-known/acme-challenge/` from that path before the HTTP→HTTPS redirect, otherwise renewal gets `404` and the cert expires. Do not put `return 301` at the HTTP `server` level because it can bypass the ACME `location`; put the redirect inside `location /`. Keep `deploy/nginx-vladexecute.ru.conf` in sync with the VPS config.

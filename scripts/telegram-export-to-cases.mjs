...
if (!exportPath) {
  console.error('Укажите путь к result.json: node telegram-export-to-cases.mjs <path-to-result.json>');
  process.exit(1);
}
...
function isCaseLike(text) {
  ...
  const hasSolution = /(решение|что сделал|собрал|настроил|как это работает)/.test(lower);
...
console.log(`Всего сообщений: ${messages.length}, подходящих под кейсы: ${caseMessages.length}\n`);
...
for (const msg of caseMessages) {
  ...
  const date = msg.date || '';
  const id = msg.id;
  const firstLine = text.split('\n').find((l) => l.trim().length > 0)?.trim().slice(0, 80) || `message_${id}`;
  caseList.push({ id, date, firstLine });
  const safeName = `case-${id}-${date.slice(0, 10)}.txt`;
  writeFileSync(join(outDir, safeName), `date: ${date}\nid: ${id}\n\n${text}`, 'utf-8');
}
...
console.log('Список кейсов:', caseList.map((c) => `${c.id} ${c.date} ${c.firstLine}`).join('\n'));
